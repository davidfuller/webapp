class ScheduleComparisonsController < ApplicationController
  ...
  def index
	...
	if params[:show] == 'all'
      @schedule_comparisons = ScheduleComparison.find_all_by_schedule_file_id(params[:schedule_filename])
  ...
  def delete_all_schedule_comparisons
    ScheduleComparison.delete_all
    flash[:notice] = "All Schedule Comparison data deleted"
    respond_to do |format|
      format.html { redirect_to(schedule_comparisons_url) }
      format.xml  { head :ok }
	end
